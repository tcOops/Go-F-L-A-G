...
```
关于Https的一些理解
这一点老是遗忘， https加密传输基于RSA公钥密钥。 我有一个公钥和密钥，将公钥公开给别人，别人用这个公钥对数据加密之后进行传输， 只有有密钥的人才能解开。
过程：
1.浏览器发起一个https请求到服务器端
2.服务器端有一套证书系统， 通常是CA中心签发的， 然后将证书传给浏览器端， 实际上传的是公钥。
3.为什么要CA中心签发？ 因为第2步可能会被SSL劫持之类， 然后伪造一个假证书， 来伪造服务器端跟浏览器端进行后续通信， 此时是不安全的，所以必须需要权威机构进行验证， 而
浏览器端已经知道哪些证书是合法的。 这样浏览器端就正确得到了公钥。
4.浏览器得到公钥之后， 随机生成一个数字，用这个公钥进行加密传输给服务器端， 服务器端用密钥解开就得到了这个随机值， 之后两边都用这个随机值进行传输。
5.服务器端将content混合随机值进行传输， 而浏览器端知道这个随机值可以进行数据还原。
```
